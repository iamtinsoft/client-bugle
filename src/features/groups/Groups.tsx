...
const Groups = () => {
  let groupProperties = ["S/N", "Name", "Description", "Actions"];
    ...
    if (searchText.length > 2) {
      ...
      setUsers(() =>
        AllUsers.filter((user: any) =>
          user.groupName.toLocaleLowerCase().startsWith(lowerCaseSearchText)
        )
      );
        ...
        <Box my={2}>
          ...
          >
            {users && users.length > 0 && !isLoading && (
              <UsersTable data={users} properties={groupProperties} />
            )}
